...
if [[ ${1:-} == "--all" ]]; then
  ...
  while IFS= read -r -d '' path; do
    ...
    case "${path}" in
      ...
      scripts/stage-server-release.sh | scripts/check-deployment-storage.py)
        homeserver=true
        integration=true
        ;;
      ...
      tests/test_server_health.py | scripts/factorio-*.py | tests/test_factorio_*.py | tests/factorio-runtime.py)
        homeserver=true
        ;;
