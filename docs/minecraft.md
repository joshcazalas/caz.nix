...
[hTreecapitator](https://modrinth.com/plugin/htreecapitator) through the container's
[built-in Modrinth support](https://docker-minecraft-server.readthedocs.io/en/latest/mods-and-plugins/modrinth/).
On each container startup, `MODRINTH_PROJECTS=htreecapitator` selects the latest
stable release marked compatible with the configured Minecraft version and
Paper, downloads it, and cleans up the previous managed version. There is no
plugin URL or hash to update, and no separate updater job. Updating the plugin
does not upgrade Minecraft itself.

Gameplay settings are generated by Nix and mounted read-only. Omitted settings,
including messages, use the installed plugin's bundled defaults. Plugin updates
can arrive on any restart without a PR; rolling back Nix alone does not roll
back the plugin. If a release causes trouble, temporarily set
`MODRINTH_PROJECTS` to `htreecapitator:<known-good-version-number>` in the module
and deploy. Startup requires access to Modrinth, and compatibility labels do
not replace a gameplay check after an update.

Sneak (hold Shift) and break a log with an axe to chop the connected tree.
No client mod or special enchantment is required. The configuration sets
a 128-block limit and progressive breaking, with the plugin's tool durability costs
